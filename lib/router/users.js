...
UserPageController = RouteController.extend({
  ...
  data: function() {
    var userById = Meteor.users.findOne(this.params._idOrSlug);
    if (typeof userById !== 'undefined') {
      // redirect to slug-based URL
      Router.go(getProfileUrl(userById), {replaceState: true});
    } else {
      var userBySlug = Meteor.users.findOne({slug: this.params._idOrSlug});
      return {
        user: userBySlug
      };
    }
  ...
  getDescription: function () {
    return i18n.t('the_profile_of', {displayName: getDisplayName(this.getUser())});
  },
